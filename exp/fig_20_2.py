# Impact of different block sizes -- recovery time
...
def main(cmd: CMDManager, tp: LogParser):
    plot_data = {
        'X_data': block_sizes,
        'Y_data': {block_size: {} for block_size in block_sizes},
    }
    method = 'aceso'
    ...
    for block_size in block_sizes:
        ...
        SERVER_RECOVER  = f"{server_env_cmd} && ./server_recover {block_size}"
        MICRO_TEST = f"{client_env_cmd} && ./client_perf update-consume {CN_num} {client_num_per_CN} 8 {block_size}"
        ...
        print_GOOD(f"[FINISHED POINT] method={method} block_size={block_size} time_dict={recover_time_dict}")
        plot_data['Y_data'][block_size] = recover_time_dict
