# MN recovery time -- memory size test
...
ec_codes           = fig_params['ec_codes']

@print_func_time
def main(cmd: CMDManager, tp: LogParser):
    plot_data = {
        'X_data': ec_codes,
        'Y_data': {ec_code: {} for ec_code in ec_codes},
    }
    method = 'aceso'
    ...
    for ec_code in ec_codes:
        ...
        if ec_code == 'RS':
            cmake_option = '-DENABLE_RS_CODE=on'
        else:
            cmake_option = ''
        ...
        print_GOOD(f"[FINISHED POINT] method={method} ec_code={ec_code} time_dict={recover_time_dict}")
        plot_data['Y_data'][ec_code] = recover_time_dict
                
    # save data
    Path(output_path).mkdir(exist_ok=True)
    with (Path(output_path) / f'{fig_num}a.json').open(mode='w') as f:
