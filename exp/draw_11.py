...
data_path = "./results"
with (Path(data_path) / f'fig_11a.json').open(mode='r') as f:
    rep3_data = json.load(f)

methods = ["STORAGE", "COMPUTE", "TRANSIENT"]
fusee_micro_data = [[], []]
for method in methods:
    fusee_micro_data[0].append(rep3_data['Y_data']['fusee'][method] / 1000000)
    fusee_micro_data[1].append(rep3_data['Y_data']['aceso'][method] / 1000000)
...
ax2.yaxis.set_label_coords(-0.1, 0.95)
...
plt.savefig("./figures/fig_11.pdf", format='pdf')
plt.close()